...
function ShowcaseCard({
  item, isHovered, isNeighbor, onEnter, onLeave,
}: {
  item: typeof showcaseItems[0];
  isHovered: boolean; isNeighbor: boolean;
  onEnter: () => void; onLeave: () => void;
}) {
  const cardRef        = useRef<HTMLDivElement>(null);
  const mouseLiveRef   = useRef({ x: 0.5, y: 0.5 });
  const mouseSpringRef = useRef({ x: 0.5, y: 0.5 });
  const magnetSpringRef= useRef({ x: 0, y: 0 });
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    mouseLiveRef.current = { x: (e.clientX - rect.left) / rect.width, y: (e.clientY - rect.top) / rect.height };
  }, []);
  ...
  return (
    <div style={{
      ...
      >
        <canvas ref={particleCanvas} width={620} height={560} style={{
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(4,4,10,0.98) 0%, rgba(4,4,10,0.25) 52%, transparent 100%)' }} />

        <div style={{
          position: 'absolute', inset: 0,
          background: `radial-gradient(circle at ${renderMouse.x * 100}% ${renderMouse.y * 100}%, rgba(${item.accentRgb},0.18) 0%, transparent 55%)`,
          opacity: isHovered ? 1 : 0, transition: 'opacity 0.4s ease', mixBlendMode: 'screen',
        }} />
        ...
        <div style={{
          ...
          transition: 'box-shadow 0.5s ease', pointerEvents: 'none', zIndex: 6,
        }} />

        <div style={{
          position: 'absolute', bottom: 0, left: 0, right: 0,
          padding: isHovered ? '2rem' : '1.75rem',
          transition: 'padding 0.5s cubic-bezier(0.16,1,0.3,1)', zIndex: 8,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.85rem' }}>
            <div style={{
              ...
              color: item.accent, transition: 'all 0.5s ease',
            }}>
              <span style={{
                width: '5px', height: '5px', borderRadius: '50%', background: item.accent,
                boxShadow: isHovered ? `0 0 8px ${item.accent}` : 'none', flexShrink: 0,
            ...
            <span style={{
              fontSize: '9px', letterSpacing: '0.15em', color: `rgba(${item.accentRgb},0.4)`,
          ...
          <h3 style={{
            fontSize: isHovered ? '1.55rem' : '1.2rem', fontWeight: 800, letterSpacing: '-0.03em',
          ...
          <div style={{
            overflow: 'hidden', maxHeight: revealed ? '200px' : '0px', opacity: revealed ? 1 : 0,
            ...
            <div style={{
              ...
              transform: revealed ? 'scaleX(1)' : 'scaleX(0)', transformOrigin: 'left',
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
              <span style={{
                ...
                transform: revealed ? 'translateY(0)' : 'translateY(14px)', display: 'inline-block',
        ...
        {[
          ...
          <div key={ci} style={{
            ...
            top: c.top, bottom: (c as any).bottom, left: (c as any).left, right: (c as any).right,
        ...
        <div style={{
          ...
          left: `calc(${renderMouse.x * 100}% - 5px)`, top: `calc(${renderMouse.y * 100}% - 5px)`,
          width: '10px', height: '10px', borderRadius: '50%', background: item.accent,
          boxShadow: `0 0 16px 4px rgba(${item.accentRgb},0.5)`,
          opacity: isHovered ? 0.9 : 0, transition: 'opacity 0.3s ease', mixBlendMode: 'screen',
        }} />
        <div style={{
          position: 'absolute', zIndex: 19, pointerEvents: 'none',
          left: `calc(${renderMouse.x * 100}% - 22px)`, top: `calc(${renderMouse.y * 100}% - 22px)`,
...
export default function ShowcaseSection() {
  ...
  /* ── GSAP scroll entrance with Lenis awareness ── */
  useEffect(() => {
    ...
    void (async () => {
      ...
      // "View more works" button entrance
      if (btnRef.current) {
  ...
  useEffect(() => {
    ...
    const ARROW_MAX  = 6;
    const ARROW_ACCEL = 0.18;
    const ARROW_DECAY = 0.88;
    let lastTime = 0;
      ...
      <div ref={btnRef} style={{ display: 'flex', justifyContent: 'center', marginTop: '16px', position: 'relative', zIndex: 10 }}>
        <a
          ...
          onMouseEnter={(e) => {
            ...
            el.style.background  = 'rgba(201,169,110,0.13)';
            el.style.color       = '#C9A96E';
            el.style.boxShadow   = '0 0 32px rgba(201,169,110,0.15)';
          }}
          onMouseLeave={(e) => {
            const el = e.currentTarget;
            el.style.borderColor = 'rgba(201,169,110,0.28)';
            el.style.background  = 'rgba(201,169,110,0.06)';
            el.style.color       = 'rgba(237,233,227,0.75)';
